...
def test_scheduler_triggers_at_processing_minute() -> None:
    assert scheduler.TRIGGER_MINUTE == 45
    assert scheduler.TRIGGER_MINUTE == scheduler.PROCESSING_TRIGGER_MINUTE


def test_scheduler_at_1045_passes_1100_boundary(monkeypatch) -> None:
    calls = []
    ...
    scheduler._run_pipeline(datetime(2026, 7, 17, 10, 45, tzinfo=UTC))
    command = calls[0][0]
    assert command[command.index("--boundary") + 1] == "2026-07-17T11:00:00Z"
    assert command[command.index("--editions") + 1] == "en,es"


def test_scheduler_at_2345_passes_next_day_boundary(monkeypatch) -> None:
    calls = []
    monkeypatch.setattr(
        scheduler.subprocess,
        "run",
    ...
    scheduler._run_pipeline(datetime(2026, 7, 17, 23, 45, tzinfo=UTC))
    command = calls[0][0]
